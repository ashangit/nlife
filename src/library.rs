...
pub struct LibraryEntry {
    ...
    pub rle: &'static str,
    /// Description from `#C` comment lines in the `.rle` file (newline-separated).
    pub description: Option<&'static str>,
    /// Author from the `#O` comment line in the `.rle` file.
    pub author: Option<&'static str>,
    /// Rule string from the `rule = …` header in the `.rle` file (e.g. `"B3/S23"`).
    ...
    DECODED.get_or_init(|| {
        LIBRARY
            ...
            .filter_map(|entry| {
                parse_rle(entry.rle)
                    .ok()
                    .map(|p| (entry.clone(), center_cells(p.cells)))
            })
            .collect()
    })
}
